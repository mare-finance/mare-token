...
contract OwnedDistributor is Distributor {
    ...
    constructor(address mare_, address claimable_, address admin_) Distributor(mare_, claimable_) {
        admin = admin_;
    }
    ...
    function editRecipients(address[] memory accounts, uint256[] memory shares) public virtual {
        require(msg.sender == admin, 'OwnedDistributor: UNAUTHORIZED');
        require(accounts.length == shares.length, 'OwnedDistributor: INVALID_INPUT');
