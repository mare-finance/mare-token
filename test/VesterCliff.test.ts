...
const deployFixture = async () => {
    ...
    await (
        await mare.connect(admin).transfer(vesterCliff.address, vestingAmount)
    ).wait(1)

    // Distributor
    const OwnedDistributor = await ethers.getContractFactory('OwnedDistributor')
...
describe('Vester Cliff', function () {
    ...
    it('Should deploy contract correctly', async function () {
        const { vesterCliff, distributor, mare } = await loadFixture(deployFixture)
    ...
    it('Should vest correctly', async function () {
        const { mare, vesterCliff, distributor, rec1, rec2 } =
            await loadFixture(deployFixture)

        let _rec1Balance = await mare.balanceOf(rec1.address)
        let rec1Balance_
        ...
        rec1Balance_ = await mare.balanceOf(rec1.address)
        ...
        rec2Balance_ = await mare.balanceOf(rec2.address)
